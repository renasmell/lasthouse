...
const inHousing = () => /HOUSING/.test(Scoreboard.getTitle())
...
function getHouseInfo(formatted=false) {
  ...
  return { name: footer[1], owner: formatted ? footer[2] : removeRanks(footer[2]), id: serverID.replace(/m/i, '') };
}
...
register('command', () => {
  ...
  function makeHouseComponent(house) {
    let comp = new TextComponent(`${house.name} by ${house.owner}\n`)
  ...
  last_houses.forEach(house => list.addTextComponent(makeHouseComponent(house)));
